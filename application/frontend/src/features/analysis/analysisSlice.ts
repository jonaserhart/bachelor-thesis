...
export const getMyModels = createAsyncThunk(
    prefix('getMyModels'),
    async function () {
        var response = await axios.get<AnalysisModel[]>('/devops/mymodels');
...
export const createModel = createAsyncThunk(
    ...
    async function (model: { name: string, project: Project }) {
        var response = await axios.post<AnalysisModel>('/devops/createmodel', model);
        return response.data;
    }
);
...
export const updateModelDetails = createAsyncThunk(
    ...
    async function (changedModel: AnalysisModelChange) {
        var response = await axios.put<AnalysisModel>(`/devops/updatemodel/${changedModel.id}`, changedModel);
        return response.data;
    }
);
...
export const getModelDetails = createAsyncThunk(
    ...
    async function (modelId: string) {
        var response = await axios.get<AnalysisModel>(`/devops/getmodel/${modelId}`);
        return response.data;
    }
);
...
export const selectModel = (id: string) => (state: RootState) => state.analysis.models.find(x => x.id === id);
